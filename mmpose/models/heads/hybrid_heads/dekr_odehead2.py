...
class DEKRODEHead2(BaseHead):
    ...
    def __init__(self,
        ...
        # ode
        # self.ODE_layer = nmodeblock(self.in_channels, self.in_channels)
    ...
    def _make_heatmap_conv_layers(self, in_channels: int, out_channels: int,
        ...
        layers = [
            ConvModule(
                ...
                kernel_size=1,
                norm_cfg=dict(type='BN')),
            nmodeblock(num_filters, num_filters),
...
class nmODE(nn.Module):
    ...
    def forward(self, t, p):
        self.nfe = self.nfe + 1
        # dpdt = -p + torch.pow(torch.sin(p + self.gamma.clone()), 2)
        # dpdt = -p + self.relu(p + self.gamma)
